<div>
    ...
    <form action="{{ route('evaluasi.update',['id_pegawai' => $id_pegawai, 'id_tahun_ajaran' => $selectedTahunAjaran]) }}" method="post">
        @csrf @method('put')
        ...
        <textarea name="catatan" class="form-control" placeholder="Catatan jika ada" rows="5">{{  $dataEvaluasi?->first()?->catatan ?? '-' }}</textarea>
        @can('manajemen_rekap_evaluasi.update')
            <div class="row mt-3">
